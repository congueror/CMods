        ...
        if (mc != null) {
            consumer.accept(new IItemRenderProperties() {
                ...
                public void renderHelmetOverlay(ItemStack stack, Player player, int width, int height, float partialTicks) {
                    ...
                    //Tanks
                    if (obj != null && !obj.getBreathable()) {
                        ArrayList<Integer> tankAmount = new ArrayList<>();
                        ArrayList<Integer> tankCapacity = new ArrayList<>();
                        Arrays.stream(SpaceSuitUtils.getTanks(player))
                                .forEach(itemStack -> {
                                    tankAmount.add(((OxygenTankItem) itemStack.getItem()).getFluid(itemStack).getAmount());
                    ...
                    //Protection Bars
                    if (obj != null && (!obj.isRadiationSafe() || !obj.isDayTemperatureSafe() || !obj.isNightTemperatureSafe())) {
                    ...
                    if (obj != null) {
                        ...
                        String rad = "Radiation: " + radiation + " Sv/y";
                        if (CGConfig.RADIATION.get() == CGConfig.RadiationUnits.REM) {
                            rad = "Radiation: " + new DecimalFormat("#.00").format(MathHelper.sievertsToRem(radiation)) + " rem";
                        }
                    ...
                    if (obj != null) {
                        ...
                        int trueTemp = SpaceSuitUtils.getTemperature(level);
                        ...
                        String temp = "Temperature: " + new DecimalFormat("#.00").format(temperature) + " \u00b0C";
                        if (CGConfig.TEMPERATURE.get() == CGConfig.TemperatureUnits.KELVIN) {
                            temp = "Temperature: " + new DecimalFormat("#.00").format(MathHelper.celciusToKelvin(temperature)) + " K";
                        } else if (CGConfig.TEMPERATURE.get() == CGConfig.TemperatureUnits.FARRENHEIT) {
                            temp = "Temperature: " + new DecimalFormat("#.00").format(MathHelper.celciusToFarrenheit(temperature)) + " \u00b0F";
                        }

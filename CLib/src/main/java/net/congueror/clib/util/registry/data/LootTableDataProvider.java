...
public class LootTableDataProvider extends LootTableProvider implements DataProvider {
    ...
    /**
     * Creates a loot table which will drop multiple block items when the block state has met the passed in predicate. Otherwise, drops a single block item.
     *
     * @param block The {@link Block} which the loot table will be applied to.
     * @param state e.g. StatePropertiesPredicate.Builder.properties().hasProperty(CarrotBlock.AGE, 7)
     */
    public void createCropDrop(Block block, StatePropertiesPredicate.Builder state) {
        LootItemCondition.Builder l = LootItemBlockStatePropertyCondition.hasBlockStateProperties(block).setProperties(state);
        addTable(block.getLootTable(), applyExplosionDecay(block, LootTable.lootTable().withPool(LootPool.lootPool()
